...
indexCtrl.PostRandomMap = async (req, res) => {
  ...
  const Beatmaps = await Request.Api().getUndocumented(
    "/beatmapsets/search?" + (type ? "m=" + type : "")
  );
  const RandomMap = RandomKeys(Beatmaps.beatmapsets, 3);

  res.send({ maps: RandomMap });
};

function RandomKeys(arr, repeat) {
  var getRandom = "";
  var prevRandom = "";
  var NewArray = [];
  ...
  for (let index = 0; index < repeat; index++) {
    ...
    if (prevRandom !== getRandom && arr[getRandom]) {
      arr[getRandom].beatmaps.filter((e) => {
        ...
        if(!NewArray.includes(calc)){
          e.difficulty_rating_calc = calc
          e.difficulty_rating_color = helpers.getDiffColour(e.difficulty_rating);
        }
      })
      arr[getRandom].beatmaps.sort((a,b) => a.difficulty_rating - b.difficulty_rating);
